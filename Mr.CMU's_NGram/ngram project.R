...
txt <- "Mr. and Mrs. Dursley, of number four, Privet Drive, were proud to say that they were perfectly normal, thank you very much. They were the last people you'd expect to be involved in anything strange or mysterious, because they just didn't hold with such nonsense. 
...
string.summary(str)
ng <- ngram (str, n = 2)
prob <- get.phrasetable(ng)
prob

numWords <- readline(prompt = "How many words are in your short story?")
intWords <- as.integer(numWords)
for (i in numWords) {
  input <- readline(prompt = "Enter your word")
  write(input, file = storyTxt)
  #search(input)
  close(storyTxt)
}
